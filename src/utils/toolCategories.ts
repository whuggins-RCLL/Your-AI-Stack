...
const buildSource = (tool: ToolLike) =>
...
export const isPodcastTool = (tool: ToolLike) => {
  ...
  const hasPodcastTag = (tool.tags || []).some((tag) => tag.toLowerCase() === 'podcasts');
...
export const getCategoryOptions = (tools: ToolLike[] = []) => {
...
export const matchesQuickFilterCategory = (tool: ToolLike, quickFilter: string) => {
